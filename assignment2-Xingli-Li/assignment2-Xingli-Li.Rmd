...
（ii）Print the summary of(ii) Why? (1 -- 2 sentences.) the regression model created in (b)(i) and comment on how well the model fits the data by studying the R2 , RSE and RMSE. (Print out the values of R2 , RSE, and RMSE.)
...
```{r}
```{r}
print("Total observations correctly predicted:")
```

（f） Comment on the results of (d) and (e):

（g） Which of (d) or (e) results in more matches?
