...
get_results <- function(model, pred, test.df) {
  ...
  acc <- performance(f.pred, measure = "acc")
  acc <- a$byClass["Balanced Accuracy"]
...
for (model in model.list) {
  ...
  pre <- predict(model, newdata = df.test.cleaned)

  confuseTable <- table(pre, df.test.cleaned$income, dnn = c("prediction", "actual"))
