...
model %>% layer_dense(units = 128, activation = 'relu', input_shape = c(,3)) %>%  # hidden layer

layer_dropout(rate = 0.3) %>%    # dropout
...
```
 training
```{r}
train.X <- c(train.df$xaccel,train.df$yaccel,train.df$zaccel)
...
```
 evaluate model
```{r}
test.X <- c(test.df$xaccel,test.df$yaccel,test.df$zaccel)
