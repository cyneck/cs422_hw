...
Answer:
    ...
    I think the appropriate number of clusters is 15.
...
```{r}
km <- kmeans(df.standardized, centers=15,nstart=25)
...
Answer:
    ...
    I think that the value of MinPts is 4 for this dataset. Because this is 2 dimensions. MinPts usually equal 2*dimensions.
...
为了找到 ɛ (eps) 的值，我们需要计算每个点到它的 k 个最近邻居的平均距离。 将 k 的值设置为您在 (e)(i) 中获得的结果。 然后，使用该值确定 ɛ 的正确值应该是多少。 （提示：查看函数 kNNdistplot() 的在线手册页）。
...
dbscan(df.standardized,eps,minPts)
```

Answer:

    At minPts = 4, eps = 0.06, there are 29 clusters.
